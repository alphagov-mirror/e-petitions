  ...
  context "when parliament is dissolving" do
    ...
    context "and the email address is a creator" do
      ...
      it "only sends the PetitionMailer#notify_creator_of_closing_date_change email" do
        expect(PetitionMailer).to receive(:notify_creator_of_closing_date_change).with(signature, created_petitions).and_call_original
        expect(PetitionMailer).not_to receive(:notify_signer_of_closing_date_change).with(signature, petitions).and_call_original
    ...
    context "and the email address is a signer" do
      ...
      it "only sends the PetitionMailer#notify_signer_of_closing_date_change email" do
        expect(PetitionMailer).not_to receive(:notify_creator_of_closing_date_change).with(signature, created_petitions).and_call_original
        expect(PetitionMailer).to receive(:notify_signer_of_closing_date_change).with(signature, petitions).and_call_original
        expect(notification).to receive(:touch).and_call_original
